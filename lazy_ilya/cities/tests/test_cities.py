...
class CitiesViewTest(TestCase):
    def setUp(self):
        ...
        self.user = CustomUser.objects.create_user(username='testuser', password='testpass',phone_number='+79852000338')
        self.client = Client()
    ...
    def test_get_cities(self):
        ...
        self.assertIn('cities_json', response.context)  # предположим, что контекст содержит 'cities'

    def test_put_update_city_success(self):
        url = reverse('cities:edit_city', args=[self.city.table_id.id, self.city.dock_num])
        data = {
    ...
    def test_put_update_city_not_found(self):
        ...
        self.assertJSONEqual(response.content, {"status": "error", "message": "Город не найден"})

    def test_put_update_city_bad_json(self):
        url = reverse('cities:edit_city', args=[self.city.table_id.pk, self.city.dock_num])
        bad_json = "{bad json}"
        response = self.client.put(
            url,
            data=bad_json,
        ...
        self.assertJSONEqual(response.content, {"status": "error", "message": "Неверный формат JSON"})
    ...
    def test_delete_city_success_with_counter(self, mock_counter_get):
        ...
        url = reverse('cities:delete_city', args=[self.city.table_id.id, self.city.dock_num])
    ...
    @patch('cities.models.CounterCities.objects.get', side_effect=CounterCities.DoesNotExist)
