...
class GreaterViewTests(TestCase):
    ...
    def test_post_upload_files_success(self, mock_parser, mock_converter) -> None:
        ...
        mock_converter.return_value.convert_files.return_value = (
            None  # Имитация успешной конвертации
        )

        # Настройка мока для Parser
        mock_parser.return_value.create_file_parsed.return_value = (
            self.test_str
        )  # Имитация возвращаемого содержимого
        ...
        self.assertEqual(
            json_response["content"], self.test_str
    ...
    def test_post_upload_files_success2(self) -> None:
        ...
        expected_new_file_name = (
            "1_test_file.txt"  # Замените на ожидаемое имя файла после обработки
        )
    ...
    def test_post_upload_files_invalid_document_number(self) -> None:
        ...
        json_response = json.loads(response.content)
        self.assertEqual(
            json_response["error"], "Номер документа должен быть больше нуля"
        )
    ...
    def test_post_upload_files_file_save_error(self) -> None:
        ...
        with patch(
            "work_for_ilia.views.OverwritingFileSystemStorage.save",
...
class CitiesTests(TestCase):
    def setUp(self):
        self.client = Client()
        self.url = reverse(
            "work_for_ilia:cities"
        )  # Замените 'cities' на имя вашего URL
        self.superuser = (
    ...
    def test_post_success(self):
        ...
        with patch("work_for_ilia.views.OverwritingFileSystemStorage") as mock_fs:
            ...
            mock_file.name = (
                "globus.docx"  # Убедитесь, что имя файла соответствует ожидаемому
            )
            ...
            self.assertJSONEqual(
                response.content, {"message": "File uploaded successfully"}
            )
    ...
    def test_process_file_success(self, mock_bulk_create, mock_document):
        ...
        mock_document.return_value.paragraphs = [
            MagicMock(text="City1"),
            MagicMock(text="City2"),
        ]
    ...
    def test_get_cities_json(self):
        ...
        expected_data = [
            {"table_name": "Moscow"}
        ]  # Предполагается, что у вас есть метод to_dict() в модели
