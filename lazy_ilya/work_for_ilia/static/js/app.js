...
class DocumentConverter {
    ...
    /**
     * Метод для навигации по файлам:
     *  Создайте метод navigateFiles, который будет изменять выделение и отображать содержимое:
     */
    navigateFiles(direction) {
        const fileNames = Array.from(this.files.keys());
        if (fileNames.length === 0) return; // Если нет файлов, ничего не делаем

        // Рассчитываем новый индекс
        this.currentIndex = (this.currentIndex + direction + fileNames.length) % fileNames.length;

        const selectedFileName = fileNames[this.currentIndex];

        // Обновляем выделение в интерфейсе
        document.querySelectorAll('.file-preview-item').forEach((item, index) => {
            item.classList.toggle('active', index === this.currentIndex);
        });

        // Отображаем содержимое выбранного файла
        this.showFilePreview(selectedFileName);
    }
    ...
    updateFilesList() {
        ...
        this.currentIndex = -1; // Сбрасываем индекс
        ...
        this.files.forEach((fileData, fileName) => {
            ...
            // Обработчик клика для отображения содержимого файла
            fileItem.addEventListener('click', () => {
                document.querySelectorAll('.file-preview-item').forEach(item => item.classList.remove('active'));
                fileItem.classList.add('active');
                this.showFilePreview(fileName);
                this.currentIndex = Array.from(this.files.keys()).indexOf(fileName); // Обновляем текущий индекс
            });
    ...
    showFilePreview(fileName) {
        if (fileName && this.files.has(fileName)) {
            ...
            // Сбрасываем состояние навигации, когда пользователь редактирует текст
            this.textPreview.addEventListener('focus', () => {
                this.isNavigating = false; // Отключаем навигацию при фокусе на textarea
            });

            this.textPreview.addEventListener('blur', () => {
                this.isNavigating = true; // Включаем навигацию при потере фокуса
            });
        } else {
