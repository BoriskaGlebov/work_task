...
class TestGreater(TestCase):
    ...
    def test_post_request_valid_files(self):
        ...
        with open(file_path1, "rb") as f1, open(file_path2, "rb") as f2:
            response = self.client.post(
                reverse("work_for_ilia:index"),
                data={"document_number": 1, "file": [f1, f2]},
            )
            self.assertEqual(response.status_code, 200)
    ...
    def test_post_request_no_files(self):
        ...
        response = self.client.post(
            reverse("work_for_ilia:index"), data={"document_number": 1}
        )
    ...
    def test_post_request_invalid_document_number(self):
        ...
        with open(file_path, "rb") as f:
            response = self.client.post(
                reverse("work_for_ilia:index"),
                data={"document_number": 0, "file": f},
            )

        self.assertEqual(response.status_code, 400)
        self.assertIn("error", response.json())
        self.assertEqual(
            response.json()["error"], "Номер документа должен быть больше нуля"
        )
