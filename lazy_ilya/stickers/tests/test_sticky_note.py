...
class StickyNoteViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(username='testuser12345', password='pass123', first_name='Иван',
                                             last_name='Иванов', phone_number=settings.ALLOWED_PHONE_NUMBERS[-1])
        self.note = StickyNote.objects.create(owner=self.user, text="Тест", author_name="Иван")
    ...
    def test_post_creates_note(self):
        ...
        response = self.client.post(reverse('stickers:stickers'), data=json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, 201)
        self.assertEqual(StickyNote.objects.count(), 2)

    def test_post_invalid_json(self):
        self.client.login(username='testuser12345', password='pass123')
        response = self.client.post(reverse('stickers:stickers'), data='not-json', content_type='application/json')
    ...
    def test_patch_updates_note(self):
        ...
        response = self.client.patch(reverse('stickers:stickers'), data=json.dumps(patch_data),
    ...
    def test_delete_note(self):
        self.client.login(username='testuser12345', password='pass123')
        response = self.client.delete(reverse('stickers:delete_stickers', args=[self.note.id]))
        self.assertEqual(response.status_code, 200)
